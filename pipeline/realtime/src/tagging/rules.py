...
# clustering/18_confidence_threshold.py 측정 결과:
#   물품 0.20  커버리지 89.1% 정확도 0.871 (임계값 0이면 0.818)
#   용역 0.15  커버리지 92.6% 정확도 0.879 (임계값 0이면 0.836)
# 이 곡선은 코드가 있는 공고에서 잰 것이라 낙관적이다. 실제 대상은 코드가 없는
# 공고라 미분류 비율이 더 높을 것이다. confidence를 저장해두므로 임계값을
# 바꿔도 재추론은 필요 없다.
THRESHOLD = {"thng": 0.20, "servc": 0.15, "frgcpt": 0.20}
