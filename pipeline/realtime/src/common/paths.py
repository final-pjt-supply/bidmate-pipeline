...
형식: raw/downloads/{stage}/year=Y/month=M/day=D/biz_div=TYPE/{bid_id}/{file_id}.{ext}
...
def parse_raw_key(key: str) -> dict:
    """raw key에서 stage/year/month/day/biz_div/bid_id/file_id/ext를 꺼낸다. 형식이 다르면 ValueError."""
...
def raw_key_to_text_key(key: str) -> str:
    ...
    return (
        "text_extracted/downloads/{stage}/year={year}/month={month}/day={day}"
        "/biz_div={biz_div}/{bid_id}/{file_id}.json"
    ).format(**parts)
